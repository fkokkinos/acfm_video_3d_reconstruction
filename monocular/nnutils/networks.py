...
class G_Res_Net(nn.Module):
    def __init__(self, input_features, hidden=128, output_features=3):
        ...
        self.gc9 = GCN(hidden, output_features)
        self.hidden = hidden
        self.relu = nn.LeakyReLU()

    def forward(self, features, adj):
        x = (F.elu(self.gc1(features, adj)))
        x = (F.elu(self.gc2(x, adj)))
        # features /= 2.
        # 2
        x = (F.elu(self.gc3(x, adj)))
        x = (F.elu(self.gc4(x, adj)))
        # features /= 2.
        # 3
        x = (F.elu(self.gc5(x, adj)))
        x = (F.elu(self.gc6(x, adj)))
        # features /= 2.

        # 4
        x = (F.elu(self.gc7(x, adj)))
        x = (F.elu(self.gc8(x, adj)))
        # features /= 2.

        # 5
        coords = self.gc9(x, adj)

        return coords, features.squeeze(0)
