...
class VisRenderer(object):
    ...
    def __init__(self, img_size, num_verts, faces, t_size=3):
        ...
        default_tex = np.ones((1, num_verts,3))
    ...
    def __call__(self, verts, cams=None, texture=None, rend_mask=False, atlas=True):
        """
        ...
        if rend_mask:
            rend = self.renderer.forward(verts, self.faces, cams, atlas=atlas)
            rend = rend.repeat(3, 1, 1)
            rend = rend.unsqueeze(0)
        else:
            rend = self.renderer.forward(verts, self.faces, cams, texture, atlas=atlas)[0]
    ...
    def diff_vp(self,
        ...
        rend_img = self.__call__(verts, cams=new_cam, texture=texture, atlas=atlas)
        if kp_verts is None:
            return rend_img
        else:
