...
def compute_boundaries(masks):
    bds = []
    for m in masks:
        bd = find_boundaries(m)
        bd = np.transpose(bd.nonzero())
        bds.append(bd)

    max_bd = max([bd.shape[0] for bd in bds])
    bds_final, bds_mask = [], []
    for bd in bds:
        bd_mask = np.ones(max_bd)
        bd_mask[bd.shape[0]:] = 0
        bds_mask.append(bd_mask)
        bd_final = np.zeros((max_bd, 2))
        bd_final[:bd.shape[0]] = bd
        bds_final.append(bd_final)
    bds_final = np.array(bds_final)
    bds_final[..., 0] = (bds_final[..., 0] / masks.shape[1] - 0.5) * 2
    bds_final[..., 1] = (bds_final[..., 1] / masks.shape[2] - 0.5) * 2
    tmp = bds_final[..., 0].copy()
    bds_final[..., 0] = bds_final[..., 1].copy()
    bds_final[..., 1] = tmp
    bds_mask = np.array(bds_mask)
    bds_final = np.concatenate((bds_final, bds_mask[:, :, None]), axis=-1)
    return bds_final.astype(np.float32)
