...
class Visualizer():
    ...
    table td {width: %dpx; height: %dpx; padding: 4px; outline: 4px solid black}
                ...
                self.vis.images(images, nrow=ncols, win=self.display_id + 1,
                                padding=2, opts=dict(title=self.name + '_' + title + ' images'))
    ...
    def plot_current_lbs_points(self, points, lbs, disp_offset=12):
        cmap = plt.get_cmap('YlOrBr')
        markercolor = cmap(lbs)[:, :-1]
        markercolor = (markercolor * 255).astype(np.uint8)
        idx = disp_offset
        for label, pts in points.items():
            self.vis.scatter(
                pts, opts=dict(title=self.name + '_' + label, markersize=1, markercolor=markercolor),
                win=self.display_id + idx)
            idx += 1
